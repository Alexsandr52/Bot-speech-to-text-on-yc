...
<strong style="color:red"> !!! `Lockbox` - сервис для хранения секретов. Той информации которую нельзя раскрывать, но нужно использовать. Если вы планируете делать бота боевым, то советую все токены и подобные переменные переносить туда. Далее мы будем использовать вместо `Lockbox` переменные окружения, но алгоритм подключения тот же !!!<strong>
...
<strong> !!! Важно отметить что я редактирую существующую функцию если хотите создать новую. Создайте новый триггер из очереди и пусть он отправляет сообщения в вашу новую функцию, старый триггер и функцию нужно удалить. Почитать об этом можно [тут](https://cloud.yandex.ru/ru/docs/message-queue/tutorials/video-converting-queue#create-trigger) ps. Пункт 5 !!! <strong>
